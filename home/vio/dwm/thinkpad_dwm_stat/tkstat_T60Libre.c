...
#define BAT0_ENERGY_FULL "/sys/class/power_supply/BAT0/energy_full"
#define BAT0_ENERGY_NOW "/sys/class/power_supply/BAT0/energy_now"
#define BAT0_STATUS "/sys/class/power_supply/BAT0/status"
#define BAT0_PRESENT "/sys/class/power_supply/BAT0/present"
...
char getState() 
{
	FILE *fp;
	char state;
	if(!(fp = fopen(BAT0_STATUS, "r"))) {
		perror("fopen");
		return 0;
	}
	state = tolower(fgetc(fp));
	fclose(fp);
	return state;
}
...
int getBatRemainingPercent()
{
    unsigned int energyNow, energyFull;
    float percent;
    energyFull = readInt(BAT0_ENERGY_FULL);
    energyNow = readInt(BAT0_ENERGY_NOW);
    percent = 100 * ((float) energyNow / energyFull);
    return (int) percent;
}
...
int main() 
{
	...
	for(;;) {
		...
		// power //
		state = getState();
		remaining_percent = getBatRemainingPercent();
		fprintf(stdout, "r: %c\n", state);
		fflush(stdout);
		...
		if(state == 'd') {
			...
			len += snprintf(buf+len, sizeof(buf)-len, "BAT: %2d%% | ",
				remaining_percent);
		}
